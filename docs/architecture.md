# 아키텍처

## 시스템 개요

MustGo 웹은 Flutter 앱의 WebView에서 장소 상세 정보를 표시하기 위한 정적 웹사이트입니다.

```
┌─────────────────────────────────────────────────┐
│  📱 MustGo 앱 (Flutter)                          │
│  ┌──────────────────────────────────────────┐   │
│  │  네이티브 화면 (홈, 지도, 설정)              │   │
│  └────────────────┬─────────────────────────┘   │
│                   │ 카드 탭                      │
│                   ▼                             │
│  ┌──────────────────────────────────────────┐   │
│  │  🌐 WebView                               │   │
│  │  mustgo.heg.wtf/places/{UUID}?type=...   │   │
│  └──────────────────────────────────────────┘   │
└─────────────────────────────────────────────────┘
              │
              ▼
┌─────────────────────────────────────────────────┐
│  GitHub Pages (mustgo.heg.wtf)                  │
│  - 정적 HTML/CSS/JS                             │
│  - Naver Maps SDK                              │
└─────────────────────────────────────────────────┘
```

## 앱-WebView 통신

### 앱 → WebView

| 방식 | 용도 | 예시 |
|------|------|------|
| URL 파라미터 | 앱 모드 설정 | `?type=MUSTGO-APP` |
| URL 파라미터 | 테마 설정 | `?theme=dark` |
| JS 호출 | 동적 변경 | `window.applyTheme('dark')` |

### 앱 모드 (type=MUSTGO-APP)

앱에서 WebView로 접근 시 다음 요소가 숨겨집니다:
- 헤더 (뒤로, 공유 버튼)
- 푸터 (© 2026 HEG)
- 길찾기 버튼 (.place-actions)

### WebView → 앱

`bridge.js`의 `MustGoBridge` 객체를 통해 통신:

```javascript
MustGoBridge.bookmark(placeId)      // 북마크
MustGoBridge.share(placeId, url)    // 공유
MustGoBridge.back()                 // 뒤로가기
MustGoBridge.openExternal(url)      // 외부 링크
MustGoBridge.navigate(lat, lng)     // 길찾기
MustGoBridge.call(phone)            // 전화
```

### 메시지 형식

```json
{
  "action": "bookmark",
  "data": { "placeId": "002cdb8f-5f71-4657-b259-a2e4383336db" },
  "timestamp": 1706918400000
}
```

### 지원 채널

1. **Flutter**: `window.MustGoChannel.postMessage()`
2. **Android**: `window.MustGoAndroid.postMessage()`
3. **iOS**: `window.webkit.messageHandlers.MustGo.postMessage()`

## 다크모드

### 우선순위

1. URL 파라미터 (`?theme=dark` / `?theme=light`)
2. 시스템 설정 (`prefers-color-scheme`)

### CSS 구현

```css
:root { /* 라이트모드 변수 */ }
[data-theme="dark"] { /* 다크모드 변수 */ }
@media (prefers-color-scheme: dark) { /* 시스템 다크모드 */ }
```

## 장소 페이지 생성 파이프라인

```
Supabase DB (mustgo_places 테이블)
        │
        ▼
Claude Agent (generate-html-slider.mjs)
  - Supabase REST API로 256개 장소 조회
  - 템플릿 기반 HTML 생성
  - 이미지 슬라이더, 지도, SEO 메타 태그 삽입
        │
        ▼
GitHub Repository 커밋
  - places/{UUID}.html
  - sitemap.xml 자동 업데이트
        │
        ▼
GitHub Pages 자동 배포
```

### 파일명 규칙

```
places/{UUID}.html

예시:
  places/002cdb8f-5f71-4657-b259-a2e4383336db.html
  places/00d18859-620e-4fc1-856c-7a8d472b9551.html
```

UUID는 Supabase `mustgo_places` 테이블의 `id` 필드 값입니다.

## 이미지 슬라이더

### 구조

```html
<section class="place-hero place-slider">
  <div class="place-slider-track">
    <div class="place-slider-slide">...</div>
  </div>
  <button class="place-slider-nav prev">...</button>
  <button class="place-slider-nav next">...</button>
  <div class="place-slider-counter">1 / 3</div>
</section>
```

### 기능

- 터치 스와이프 지원 (50px 이상 드래그)
- 이전/다음 버튼 네비게이션
- 카운터 표시 (현재/전체)
- 이미지 1개일 경우 네비게이션 숨김

## 네이버 지도

### SDK 설정

```html
<script src="https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId=5sgg4izutv"></script>
```

### 지도 컨테이너

```html
<div class="place-map-container" id="map"
     data-lat="37.53484249"
     data-lng="127.00085988"
     data-name="장소명">
</div>
```

### 설정값

- 줌 레벨: 18
- 컨트롤: 모두 숨김
- 마커: 장소 위치에 자동 추가

## 길찾기 (네이버 지도 앱 연동)

길찾기 버튼 클릭 시 네이버 지도 앱 URL Scheme (`nmap://`)을 사용:

```
nmap://route/car?dlat={위도}&dlng={경도}&dname={장소명}&appname=mustgo.heg.wtf
```

- 출발지 미지정 시 현재 위치 사용
- 앱 미설치 시 앱스토어/플레이스토어로 이동
- 참고: [네이버 클라우드 URL Scheme 가이드](https://guide.ncloud-docs.com/docs/maps-url-scheme)
